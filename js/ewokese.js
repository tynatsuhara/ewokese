...
function update_autocomplete() {
    val = $('#basic-input').val().trim();
    if (val.length > 0) {
        index = Math.max(index, 0);
        $('#basic-input').typeahead('val', val.substring(index));  
    }
}
...
var substringMatcher = function() {
    return function findMatches(q, cb) {
        ...
        index = Math.max(q.lastIndexOf(" "), 0);
        word_to_match = q.substring(index).trim();
        prefix = q.substring(0, index).trim();

        // iterate through the pool of strings and for any string that
        // contains the substring `q`, add it to the `matches` array
        $.each(autocomplete, function(i, str) {
            if (str.startsWith(word_to_match)) {
                matches.push((prefix + " " + str).trim());
            }
        });
...
(function main() {
    ...
    type_ahead = $('#basic-input').typeahead({}, {
        name: 'basic-input',
